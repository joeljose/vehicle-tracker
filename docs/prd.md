...
| M8 -- Custom model training | Auto-label 10 hrs of junction video (balanced 200 min/site) with the bake-off-winning teacher (RF-DETR-M), fine-tune YOLOv8s on a single `vehicle` class, export ONNX consumed by both backends (custom backend builds its own TRT engine at startup; DeepStream nvinfer builds its engine from the same ONNX). PRD and design doc at `training/docs/`. Scope change to single-class collapse documented there (see §6.2) and in `plans/m8-custom-model-training.md`. **Phase A–F complete (v0.8.0-dev); Phase G training-set review deferred.** | M6 |
